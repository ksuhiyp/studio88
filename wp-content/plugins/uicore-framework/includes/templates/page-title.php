...
class PageTitle
{
     ...
     * @since 1.0.0
     */
    public function get_taxonomy_breadcrumb($breadcrumb, $taxonomy)
    {
        $post = get_the_ID();
    ...
    {
        if ($data = $this->data['breadcrumb'] ?? false) {
            $sep = '<i class="uicore-separator uicore-i-arrow"></i>';
            $breadcrumb = '<p class="uicore-animate ui-breadcrumb" itemscope itemtype="http://schema.org/BreadcrumbList">';
